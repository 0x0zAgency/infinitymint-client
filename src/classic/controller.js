...
const Controller = new (class {
  ...
  loadAbis() {
    this.#abis = {
      ...
      contracts: {
        Fake_InfinityMintWallet: Config.getDeployment(
          "Fake_InfinityMintWallet"
        ),
        InfinityMint: Config.getDeployment("InfinityMint"),
        [Config.deployInfo.modules.svg || Config.deployInfo.modules.controller]:
          Config.getDeployment(
            Config.deployInfo.modules.svg ||
              Config.deployInfo.modules.controller
          ),
        InfinityMintApi: Config.getDeployment("InfinityMintApi"),
        InfinityMintProject: Config.getDeployment("InfinityMintProject"),
        InfinityMintLinker: Config.getDeployment("InfinityMintLinker"),
      },
    };
  }
  ...
  async executeTransaction(
    ...
    if (args.loadGasPrice) {
      await Config.loadGasPrices().catch((error) => {
    ...
    if (parameters.length === 1) {
      return await this.#instances[contract].methods[method](parameters[0])[
        type
      ]({
        from: account.address || account,
        value,
  ...
  async createTokenURI(
    ...
    return (
      "data:application/json;base64," + Base64.encode(JSON.stringify(object))
    );
  }
  ...
  storeToken(result, source = "event", from = "mint", flags = undefined) {
    ...
    const object = {
      ...
      source,
      from,
      flags: flags || StorageController.values?.tokens[tokenId]?.flags || {},
      date: Date.now(),
      originalDate:
        StorageController.values?.tokens[tokenId]?.originalDate || Date.now(),
      validTill:
        Date.now() +
        Config.settings.cacheLength +
      ...
      token: {
        ...
        stickers: {
          ...StorageController.values.tokens[tokenId]?.token?.stickers,
        },
      },
    };
  ...
  getTokenExtraColour(token, type = "background", preverseAlpha = false) {
    ...
    switch (type) {
      ...
      case "stroke": {
        result = tinySVG.toHexFromDecimal(token.colours[pathSize + 1] || 0);
        break;
      }

      case "text": {
        result = tinySVG.toHexFromDecimal(token.colours[pathSize + 2] || 0);
        break;
      }

      case "shadow": {
        result = tinySVG.toHexFromDecimal(token.colours[pathSize + 3] || 0);
        break;
      }

      case "border_1": {
        result = tinySVG.toHexFromDecimal(token.colours[pathSize + 4] || 0);
        break;
      }

      case "border_2": {
        result = tinySVG.toHexFromDecimal(token.colours[pathSize + 5] || 0);
        break;
      }
  ...
  decodeToken(
    ...
    try {
      result.mintData = JSON.parse(this.web3.utils.hexToAscii(result.mintData));
    } catch (error) {
  ...
  // Want to add new events? check src/config.js => events array
  setupEvents(contract = "InfinityMint") {
    for (const event of Object.values(
      Config.events[contract.replace("Fake_", "")]
    )) {
      this.#instances[contract].events[event]((error, events) => {
        if (!Config.getNetwork().useAllEvents) {
  ...
  pushToEvent(event, error, events) {
    ...
    this.#events[event][events?.id || "item_" + this.getLengthOfEvent(event)] =
      [error, events];
  }
  ...
  initializeOnboardJs(chainId = undefined) {
    this.onboard = Onboard({
      dappId: Config.onboardApiKey,
      networkId: chainId || Config.requiredChainId,
      subscriptions: {
        wallet: async (wallet) => {
  ...
  async tryWallet() {
    ...
    if (
      ...
      Controller.log("[⚠️] Wallet needs onboarding", "warning");

      if (!(await Controller.onboardWallet())) {
        return false;
      }
          ...
          this.#instances[contract].events.allEvents(
            {
              ...
              to: blockNumber + Config.settings.blockRange,
            },
            (error, events) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      const interval = setInterval(() => {
        ...
        if (
          ...
          this.log("transaction event resolved: " + event, "transaction");
        ...
        if (
          ...
          this.log("transaction event rejected: " + event, "transaction");
        ...
        if (--timeout < 0) {
          this.log("transaction event timeout: " + event, "transaction");
          clearInterval(interval);
          reject("timeout");
        }
      }, 1000);
  ...
  makeFakeToken(
    ...
    const fakeToken = {
      ...
      colours: [],
      mintData: {},
      name: name || Controller.getDescription().token,
      pathId,
      pathSize: 0,
      assets: [1, 4],
    ...
    const pathSize = Controller?.paths[fakeToken.pathId]?.pathSize || 1;
    ...
    if (pathSize <= div) {
      ...
      for (let i = 0; i < groups * 2; i++) {
        if (i % 2 === 0) {
          objects[i] =
            colourSeeds[seedCount++] || Math.floor(Math.random() * 0xff_ff_ff);
        } else {
    ...
    return {
      ...
      pathSize:
        Controller?.paths[fakeToken.pathId]?.pathSize === undefined
          ? 0
          : Controller.paths[fakeToken.pathId].pathSize,
    };
  }
  ...
  async getTokens(maxCount = 64, page = 0, address = null) {
    ...
    for (const [i, element] of result.entries()) {
      ...
      if (objs[tokenId].token?.token?.owner !== address) {
        ...
        (Config.settings.saveTokenRange || 16)
      ) {
        if (tokenId > (Config.settings.saveTokenRange || 16)) {
  ...
  async callMethod(account, contract, method, args = {}) {
    args.gas = null;
    args.gasPrice = null;
    return await this.executeMethod(account, contract, method, args, 0, "call");
  }
  ...
  async sendMethod(account, contract, method, args, value = 0) {
    await this.executeMethod(account, contract, method, args, value, "send");
  }
  ...
  getProjectSettings() {
    ...
    if (!this.isWalletValid && Config.settings.useLocalProjectAsDefault) {
      settings = this.localProjectURI;
    } else if (!Config.settings.useLocalProjectURI && this.isWalletValid) {
      ...
      if (
        ...
        settings = Config.settings.useLocalProjectAsBackup
          ? this.localProjectURI
          : this.defaultProjectURI;
      }
    } else if (Config.settings.useLocalProjectURI) {
      settings = this.localProjectURI;
    }
  ...
  log(message, context = "general") {
    ...
    if (context === "error") {
      context = `⚠️${context}`;
      console.log(`%c [${context}] ${message}`, "color:darkred;font-size:16px");
    } else if (context === "warning") {
      context = `🔶${context}`;
      console.log(`%c [${context}] ${message}`, "color:orange;font-size:14px");
    } else {
  ...
  async preloadVariable(variable) {
    try {
      ...
      if (
        ...
        Date.now() < StorageController.values.preload[variable.key].validTill
      ) {
        ...
        this.#preloadVariables.validTill[variable.key] = {
          loaded: Date.now(),
          date: StorageController.values.preload[variable.key].validTill,
          variable,
        };
      ...
      this.log(error, "error");
      this.#preloadVariables.values[variable.key] = variable.default || null;
    }
  }
      ...
      try {
        for (const [key, value] of Object.entries(
          ...
          if (
            ...
            this.#preloadVariables.loading[key] = true;
            this.log("getting value from chain for " + key, "preload");
  ...
  async checkNetworkId() {
    ...
    if (
      Config.requiredChainId !== chainId ||
      Config.networks[chainId] === undefined
    ) {
      throw new Error(
        "Invalid network, please make sure the network you are connected to is " +
          Config.networks[Config.requiredChainId].name
      );
    }
  ...
  async loadObjectURI() {
    ...
    try {
      if (
        Config.settings.useLocalProjectAsDefault ||
        Config.settings.useLocalProjectURI
      ) {
  ...
  async loadPathGroups(checksumCheck = true) {
    ...
    for (const [pathId, path] of Object.entries(projectURI.paths)) {
      ...
      if (path.dontLoad || path.paths === undefined || path.paths.dontLoad) {
      ...
      if (
        ...
        if (
          ...
          (this.paths[pathId].paths.checksum === undefined ||
            this.paths[pathId].paths.checksum !== path.paths.checksum)
        ) {
          this.log(
            "checksum mismatch for " + pathId + " trashing stored value",
            "paths"
          );
          delete StorageController.values.paths[pathId];
          StorageController.saveData();
        } else {
          this.log("using saved storage data for path " + pathId, "paths");
      ...
      if (typeof path?.paths?.data !== "string") {
        this.log("Path id " + pathId + " has weird paths data", "warning");
        continue;
      }
      ...
      if (
        ...
        (path.paths.localStorage && path.paths.data.slice(0, 5) !== "/imgs")
      ) {
      ...
      if (
        ...
        (path.paths.localStorage === true &&
          !Config.settings.forceLocalContent &&
          path.paths.ipfs)
      ) {
      ...
      if (path.paths.dontStore !== true && path.paths.projectStorage !== true) {
  ...
  async deployContract(
    ...
    if (libraries === null) {
      libraries = {
        InfinityMintUtil: Config.getDeployment("InfinityMintUtil"),
      };
    }
    ...
    if (
      ...
      try {
        abi = Config.getDeployment(contract);
      } catch (error) {
   ...
   * @returns
   */
  initializeContract(address, contract, instance = true, forceAbi = undefined) {
    let abi;
    if (
      this.#abis.contracts[contract] === undefined &&
      forceAbi === undefined
      ...
      try {
        abi = Config.getDeployment(contract).abi;
      } catch (error) {
        Controller.log(error);
        throw new Error("bad contract: " + contract);
      }
          ...
          parse: async (value) => {
            ...
            if (value.includes("{")) {
              try {
                ...
                console.log(error);
                value = Config.settings.useLocalProjectAsBackup
                  ? this.localProjectURI
                  : this.defaultProjectURI;
              }
            } else {
              ...
              try {
                ...
                this.log("failed to get object uri: " + value, "warning");
                value = Config.settings.useLocalProjectAsBackup
                  ? this.localProjectURI
                  : this.defaultProjectURI;
              }
            ...
            if (Config.settings.useLocalProjectAsBackup) {
              if (value.tag === undefined) {
                value.tag = this.localProjectURI.tag || "initial";
              }

              if (value.version === undefined) {
                value.version = this.localProjectURI.version || 0;
              }
            }

            if (
              StorageController.getGlobalPreference("lastTag") !== undefined &&
              value.tag !== StorageController.getGlobalPreference("lastTag")
            ) {
              StorageController.setGlobalPreference(
                "previousTag",
                StorageController.getGlobalPreference("lastTag")
              );
              StorageController.setGlobalPreference("needsPathReset", true);
            }
            ...
            StorageController.setGlobalPreference(
              "lastVersion",
              value.version === undefined ? "unknown" : value.version
            );

            if (
              StorageController.getGlobalPreference("lastId") !== undefined &&
              value.id !== StorageController.getGlobalPreference("lastId")
            ) {
              StorageController.setGlobalPreference("needsFullReset", true);
            }

            StorageController.setGlobalPreference("lastId", value.id || 1);
