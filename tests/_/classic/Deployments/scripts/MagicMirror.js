...
//infinity mint logo
import InfinityMintLogo from '../../Resources/logo_icon_blue.png';
//import three js
...
const getContainerRect = (sizeElement, settings) => {
    ...
    let canvasHeight = settings.height || canvasWidth;
...
const registerThreeJSFramework = async (
    ...
    registerElement(id, await getImage(''), {
        onRender: (element) => {
            ...
            if (renderer !== undefined && renderer !== null) {
                renderer.setSize(
                    ...
                    canvas.height / (settings.downsampleRate3D || 2)
                );
        ...
        onInitialize: (element) => {
            ...
            let height = canvas.height / (settings.downsampleRate3D || 2);
            ...
            camera.position.z = settings.cameraPositionZ || 115;
            ...
            if (settings.disableFloor3D !== true) {
                let plane = new THREE.Mesh(
                    new THREE.PlaneGeometry(1000, 1000, 16, 1),
 ...
 */
const registerElements = async (canvas, controller, token, id, settings) => {
    ...
    if (settings.forceBackground !== undefined)
        ...
        if (
            ...
            image = await getImage(path.ipfsURL);
        else
    ...
    if (token.assets !== undefined && token.assets.length !== 0) {
...
const MagicMirror = {
        ...
        for (let i = 0; i < instances.length; i++) {
            try {
                ...
                canvas.setBackgroundColor(
                    controller.getTokenExtraColour(
                        Tokens[id] || { colours: [] },
                        'background'
                    )
                );
                ...
                controller.log('could not resize', 'warning');
    ...
    postRenderToken: async (
        ...
        let loadingImage = await getImage(
            'https://img2.cgtrader.com/items/2987948/abd3a245f5/infinity-symbol-v1-004-3d-model-low-poly-max-obj-3ds-fbx-ma-stl.jpg'
        );
        ...
        //remove styling from canvas-container element
        element.parentNode.style.width = '';
        element.parentNode.style.height = canvasHeight;
        //render the canvas
        canvas.renderAll();
        //queue.pop();
